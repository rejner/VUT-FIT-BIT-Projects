...
**/
	...
	//v tomto případě je strom prázdný, vytvořím první položku
	if (*RootPtr == NULL){
		...
		*RootPtr = malloc(sizeof(struct tBTNode));
	...
	}else{ //strom obsahuje nějaké uzly
		...
		while(*RootPtr != NULL){
			...
			if (Content < (*RootPtr)->Cont){
				...
				//jdu po levém ukazateli
				RootPtr = &(*RootPtr)->LPtr;
			}
			//klíč je větší než klíč v aktuálním uzlu
			else if (Content > (*RootPtr)->Cont){

				//jdu po pravém ukazateli
				RootPtr = &(*RootPtr)->RPtr;
			}
			//klíč se již nachází ve stromě, nic nedělám a vrátím se
			else if (Content == (*RootPtr)->Cont){
		...
		*RootPtr = malloc(sizeof(struct tBTNode));
		...
		//nastavím hodnoty listové položky
		(*RootPtr)->Cont = Content;
		(*RootPtr)->LPtr = NULL;
		(*RootPtr)->RPtr = NULL;
		return;
...
**/
		...
		while (ptr != NULL){
			...
			//zpracujeme navštívený uzel pomocí funkce BTWorkOut()
			BTWorkOut(ptr);

			//přesuneme ukazatel na uzel na zásobník ukazatelů
			SPushP(Stack, ptr);

			//jdeme dál po levém podstromu
			ptr = ptr->LPtr;
		}
...
**/
	...
	//vytvořím si zásobník ukazatelů a inicializuji jej
	tStackP stack;
	SInitP(&stack);

	//najdu položku nejvíce vnořenou vlevo
	Leftmost_Preorder(RootPtr, &stack);

	//dokud nebude zásobník prázdný, budeme si brát ukazatele ze zásobníku
	while (!SEmptyP(&stack)){

		RootPtr = STopPopP(&stack); //uložíme hodnotu z vrcholu zásobníku
		Leftmost_Preorder(RootPtr->RPtr, &stack); //uděláme Preorder pro pravý podstrom aktuálního uzlu
	}
}
...
**/
	 ...
	 while (ptr != NULL){
		 ...
		 SPushP(Stack, ptr); //uložíme ukazatel do zásobníku
		 ptr = ptr->LPtr;    //jdeme dál po levém ukazateli
	 }
...
**/	
	...
	//inicializace zásobníku ukazatelů
	 ...
	 while (!SEmptyP(&stack)){
		 ...
		 RootPtr = STopPopP(&stack);              //vezmeme si ukazatel z vrcholu zásobníku
		 BTWorkOut(RootPtr);                      //zpracujeme uzel pomocí BTWorkOut()
		 Leftmost_Inorder(RootPtr->RPtr, &stack); //jdeme zásobník naplnit uzly z pravého podstromu uzlu
	 }
}
...
**/
	 ...
	 while (ptr != NULL){
		 ...
		 SPushP(StackP, ptr);  //uložíme ukazatel na zásobník
		 SPushB(StackB, true); //uložíme indikaci, že uzel byl navštíven poprvé (pomocí hodnoty true)
		 ptr = ptr->LPtr;      //jdeme dál vlevo
	 }
}
...
**/
		...
		//Inicializa zásobníků ukazatelů a booleovských hodnot projití uzlu
		bool poprve;
		tStackP stackP; SInitP(&stackP);
		tStackB stackB; SInitB(&stackB);
		...
		while (!SEmptyP(&stackP)){
			...
			RootPtr = STopPopP(&stackP); //vezmeme si ukazatel
			poprve = STopPopB(&stackB);  //vezmeme si hodnotu projití

			if (poprve){ //uzel jsme již jednou prošli

				SPushB(&stackB, false);   //dáme informaci o druhém průchodu na zásobník
				SPushP(&stackP, RootPtr); //uložíme patřičný ukazatel na zásobník

				Leftmost_Postorder(RootPtr->RPtr, &stackP, &stackB); //jdeme udělat to stejné pro pravé podstromy
...
**/
	...
	tBTNodePtr tmpPtr;               //pomocná proměnná pro úklid
	tStackP stackP; SInitP(&stackP); //inicializace zásobníku ukazatelů

	//zavoláme Leftmost_Inorder a naplníme stack ukazateli 	 
	Leftmost_Inorder(*RootPtr, &stackP);

	while(!SEmptyP(&stackP)){

		tmpPtr = STopPopP(&stackP); //vezmeme si ukazatel ze sásobníku
		Leftmost_Inorder(tmpPtr->RPtr, &stackP); //provedeme naplnění zásobníku pro pravý podstrom uzlu
		free(tmpPtr); //uvolníme uzel
